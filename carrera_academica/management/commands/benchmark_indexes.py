...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING("Iniciando benchmark de índices...\n"))
        ...
        results.append(
            self.benchmark_query(
                ...
                lambda: list(
                    CarreraAcademica.objects.filter(estado="ACT").order_by(
        ...
        results.append(
            self.benchmark_query(
                ...
                lambda: list(
                    CarreraAcademica.objects.filter(numero_expediente__icontains="2024")
                ),
            )
        )
        ...
        results.append(
            self.benchmark_query(
                ...
                lambda: list(
                    Formulario.objects.filter(carrera_academica_id=1, estado="PEN")
                ),
            )
        )
        ...
        results.append(
            self.benchmark_query(
                ...
                lambda: list(
                    Cargo.objects.filter(estado="activo", caracter__in=["reg", "ord"])[
                        :50
                    ]
                ),
            )
        ...
        results.append(
            self.benchmark_query(
                ...
                lambda: list(Docente.objects.filter(apellido__icontains="a")[:50]),
            )
        )
    ...
    def print_results(self, results):
        ...
        # Encabezado
        self.stdout.write(f"{'Query':<40} {'Tiempo (ms)':<15} {'Queries':<10}")
        self.stdout.write("-" * 80)
