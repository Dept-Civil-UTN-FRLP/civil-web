...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.WARNING("Iniciando análisis de queries...\n"))
    ...
    def print_query_stats(self):
        ...
        if queries:
            self.stdout.write(f"Promedio por query: {total_time/len(queries):.4f}s")
            ...
            for i, q in enumerate(sorted_queries[:3], 1):
                self.stdout.write(f"{i}. {float(q['time']):.4f}s - {q['sql'][:100]}...")
